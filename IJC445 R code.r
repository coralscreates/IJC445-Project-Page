...
ggplot(timeline_data, aes(x = year, y = mean_pop, color = artist_type)) +
  ...
  theme_minimal() +
  labs(title = "Average Artist Popularity Score by Artist Type (2000-2018)",
       x = "Year",
...
ggplot(dataset_songs, aes(x = explicit_transformed, y = popularity.y, fill = explicit)) +
  ...
  theme_minimal() +
  labs(title = "Distribution of Popularity by Explicitness",
...
ggplot(scatter_data, aes(x = followers, y = popularity.y)) +
  ...
  theme_minimal() +
  labs(
    title = "Exploring the relationship between artist popularity and follower count",
